...
<div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-4">
    ...
    <form action="index.php" method="GET" class="d-flex align-items-center">
        ...
        <select class="form-select form-select-sm" id="sort-select" name="sort" onchange="this.form.submit()">
            <?php
            $sort_options = array('open_desc' => 'デフォルト (公開日順)', 'title_asc' => 'タイトル (昇順)', 'title_desc' => 'タイトル (降順)', 'author_asc' => '著者名 (昇順)', 'author_desc' => '著者名 (降順)');
...
<?php if (empty($works)): ?>
    <div class="alert alert-info" role="alert">このカテゴリに登録されている作品はありません。</div>
<?php else: ?>
            ...
            <a href="index.php?page=detail&id=<?= urlencode($work['work_id']) ?>" class="list-group-item list-group-item-action">
                <div class="d-flex w-100 align-items-center">
                    <div class="flex-shrink-0 me-3">
                        ...
                        <?php else: ?>
                            <div class="work-list-thumbnail bg-light d-flex align-items-center justify-content-center"><small class="text-muted">NoImg</small></div>
                        <?php endif; ?>
                    </div>
