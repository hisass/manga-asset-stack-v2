<?php
function get_opposite_order($order) { return ($order === 'asc') ? 'desc' : 'asc'; }
function get_sort_indicator($key, $current_key, $current_order) { if ($key === $current_key) { return ($current_order === 'asc') ? ' ▲' : ' ▼'; } return ''; }
?>
<div class="container-fluid mt-4">
    ...
    <div class="tab-content" id="adminTabContent">
        <div class="tab-pane fade <?= ($active_tab === 'works') ? 'show active' : '' ?>" id="works-tab-pane" role="tabpanel" aria-labelledby="works-tab" tabindex="0">
            <div class="card card-tab-content-top">
                <div class="card-body">
                    <form action="admin.php" method="GET" class="row g-3 align-items-center">
                        ...
                        <div class="col-md-4"><input type="search" class="form-control" name="search" placeholder="キーワード検索..." value="<?= htmlspecialchars($current_search_keyword, ENT_QUOTES, 'UTF-8'); ?>"></div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2"><button type="submit" class="btn btn-primary w-100">絞り込み</button></div>
                        <div class="col-md-3 text-end"><a href="admin.php?action=add_work" class="btn btn-success">作品追加</a></div>
                    </form>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <?php else: foreach ($works as $work): ?>
                            <tr>
                                <td class="align-middle">
                                    ...
                                    // ▼▼▼ NEWバッジ表示 ▼▼▼
                                    $new_badge = (isset($work['is_new']) && $work['is_new']) ? ' <span class="badge bg-danger">NEW</span>' : '';
                                    ?>
                                    <a href="admin.php?action=edit_work&id=<?= htmlspecialchars($work['work_id'], ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($work['title'], ENT_QUOTES, 'UTF-8'); ?></a><?= $source_badge; ?><?= $new_badge; ?>
                                </td>
            ...
            <nav aria-label="Page navigation" class="mt-4">
                <ul class="pagination justify-content-center">
                    <?php if ($current_page > 1): ?><li class="page-item"><a class="page-link" href="?action=dashboard&page_num=<?= $current_page - 1 ?>&sort=<?= htmlspecialchars($current_sort_key) ?>&order=<?= htmlspecialchars($current_sort_order) ?>&filter_category=<?= htmlspecialchars($current_filter_category) ?>&search=<?= htmlspecialchars($current_search_keyword) ?>">前へ</a></li><?php endif; ?>
                    <?php
                    $start_page = max(1, $current_page - 3); $end_page = min($total_pages, $current_page + 3);
                    if ($start_page > 1) {
                    ...
                    ?>
                    <?php if ($current_page < $total_pages): ?><li class="page-item"><a class="page-link" href="?action=dashboard&page_num=<?= $current_page + 1 ?>&sort=<?= htmlspecialchars($current_sort_key) ?>&order=<?= htmlspecialchars($current_sort_order) ?>&filter_category=<?= htmlspecialchars($current_filter_category) ?>&search=<?= htmlspecialchars($current_search_keyword) ?>">次へ</a></li><?php endif; ?>
                </ul>
            </nav>
            <?php endif; ?>
        </div>
        <div class="tab-pane fade <?= ($active_tab === 'categories') ? 'show active' : '' ?>" id="categories-tab-pane" role="tabpanel" aria-labelledby="categories-tab" tabindex="0">
            <div class="card card-tab-content-top text-end"><div class="card-body"><a href="admin.php?action=edit_category" class="btn btn-success">カテゴリ新規追加</a></div></div>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead class="table-dark"><tr><th>カテゴリ名</th><th>トップ表示数</th><th>エイリアス</th><th>作品数</th><th>操作</th><th class="text-center">並び順</th></tr></thead>
                    <tbody>
                        <?php if (empty($categories)): ?>
                            <tr><td colspan="6" class="text-center">登録されているカテゴリはありません。</td></tr>
                        <?php else: 
                            $cat_keys = array_keys($categories); $last_key_index = count($cat_keys) - 1; $current_index = 0;
                            ...
                            <tr>
                                ...
                                <td class="align-middle"><a href="admin.php?action=edit_category&id=<?= htmlspecialchars($cat['id'], ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-sm btn-outline-primary">編集</a></td>
                                <td class="align-middle text-center">
                                    <?php if ($current_index > 0): ?><a href="admin.php?action=move_category&id=<?= $cat_id ?>&direction=up" title="上へ">▲</a><?php endif; ?>
                                    <?php if ($current_index < $last_key_index): ?><a href="admin.php?action=move_category&id=<?= $cat_id ?>&direction=down" title="下へ">▼</a><?php endif; ?>
                                </td>
                            </tr>
                            <?php $current_index++; endforeach; endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
...
<style>.card-tab-content-top { border-top-left-radius: 0; border-top-right-radius: 0; }</style>