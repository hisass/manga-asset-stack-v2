<?php
// views/admin/dashboard.php (カテゴリに「トップ表示数」列を追加)

// ソート順を反転させるためのヘルパー関数
function get_opposite_order($order) {
    return ($order === 'asc') ? 'desc' : 'asc';
}
// 現在のソート順を示す記号（▲▼）を返すヘルパー関数
...
<div class="container-fluid mt-4">
    ...
    <ul class="nav nav-tabs" id="adminTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="works-tab" data-bs-toggle="tab" data-bs-target="#works-tab-pane" type="button" role="tab" aria-controls="works-tab-pane" aria-selected="true">作品管理</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="categories-tab" data-bs-toggle="tab" data-bs-target="#categories-tab-pane" type="button" role="tab" aria-controls="categories-tab-pane" aria-selected="false">カテゴリ管理</button>
        </li>
    </ul>

    <div class="tab-content" id="adminTabContent">
        
        <div class="tab-pane fade show active" id="works-tab-pane" role="tabpanel" aria-labelledby="works-tab" tabindex="0">
            ...
            <nav aria-label="Page navigation" class="mt-4">
                <ul class="pagination justify-content-center">
                    ...
                    <?php if ($current_page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="?action=dashboard&page_num=<?= $current_page - 1 ?>&sort=<?= htmlspecialchars($current_sort_key) ?>&order=<?= htmlspecialchars($current_sort_order) ?>&filter_category=<?= htmlspecialchars($current_filter_category) ?>&search=<?= htmlspecialchars($current_search_keyword) ?>">前へ</a>
                        </li>
                    <?php else: ?>
                    ...
                    <?php if ($current_page < $total_pages): ?>
                        <li class="page-item">
                            <a class="page-link" href="?action=dashboard&page_num=<?= $current_page + 1 ?>&sort=<?= htmlspecialchars($current_sort_key) ?>&order=<?= htmlspecialchars($current_sort_order) ?>&filter_category=<?= htmlspecialchars($current_filter_category) ?>&search=<?= htmlspecialchars($current_search_keyword) ?>">次へ</a>
                        </li>
                    <?php else: ?>
                        <li class="page-item disabled">
        ...
        <div class="tab-pane fade" id="categories-tab-pane" role="tabpanel" aria-labelledby="categories-tab" tabindex="0">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?php if (empty($categories)): ?>
                            <tr><td colspan="5" class="text-center">登録されているカテゴリはありません。</td></tr>
                        <?php else: ?>
